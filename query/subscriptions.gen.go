...
func newSubscription(db *gorm.DB, opts ...gen.DOOption) subscription {
	...
	_subscription.GameID = field.NewString(tableName, "game_id")
	_subscription.GameName = field.NewString(tableName, "game_name")
	...
	_subscription.Messages = subscriptionHasManyMessages{
		...
		RelationField: field.NewRelation("Messages", "models.Message"),
	}
...
func (s *subscription) updateTableName(table string) *subscription {
	...
	s.GameName = field.NewString(table, "game_name")
...
func (s *subscription) fillFieldMap() {
	...
	s.fieldMap["game_name"] = s.GameName
...
type subscriptionHasManyMessages struct {
	db *gorm.DB

	field.RelationField
}

func (a subscriptionHasManyMessages) Where(conds ...field.Expr) *subscriptionHasManyMessages {
	if len(conds) == 0 {
		return &a
	}
...
func (a subscriptionHasManyMessages) WithContext(ctx context.Context) *subscriptionHasManyMessages {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a subscriptionHasManyMessages) Session(session *gorm.Session) *subscriptionHasManyMessages {
	a.db = a.db.Session(session)
	return &a
}

func (a subscriptionHasManyMessages) Model(m *models.Subscription) *subscriptionHasManyMessagesTx {
	return &subscriptionHasManyMessagesTx{a.db.Model(m).Association(a.Name())}
}

type subscriptionHasManyMessagesTx struct{ tx *gorm.Association }

func (a subscriptionHasManyMessagesTx) Find() (result []*models.Message, err error) {
	return result, a.tx.Find(&result)
}

func (a subscriptionHasManyMessagesTx) Append(values ...*models.Message) (err error) {
...
func (a subscriptionHasManyMessagesTx) Replace(values ...*models.Message) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a subscriptionHasManyMessagesTx) Delete(values ...*models.Message) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a subscriptionHasManyMessagesTx) Clear() error {
	return a.tx.Clear()
}

func (a subscriptionHasManyMessagesTx) Count() int64 {
	return a.tx.Count()
}
